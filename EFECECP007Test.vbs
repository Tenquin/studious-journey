...
'NEED TO REPLACE OFFICE TEXT FILE
...
' ' Temp serial number for VM testing
' curSerialNumber = "1CW1WD2"
' '	D4:81:D7:A4:7C:94,1CW1WD2,1US CWEN WMWX WHFQ
...
' 'START HTA GUI	
' WinShell.Run "mshta c:\dataset\setup\gui.hta",1,False
' 	sendLog logFile,"HTA GUI has been launched. ","HTA"
'     Wscript.Sleep 5000

' 'THIS SECTION NEED TO BE TWEAKED
' 'Wait until GUI HTA releases loop.
' Do Until (curMsg ="INIT") OR (curMsg ="IN-PROGRESS")
' curMsg = ReadFile(msgFile)
' Wscript.Sleep(2000)
' 'Break loop if HTA is no longer there
' if IsCMDRunning("gui.hta") = False Then
...
Else
    errorFlag = True
    sendLog logFile,"FAILED to copy OEM Logo into Windows default directory.","OEM_COPY"
End if
...
Function CreateAFile(strFileName,strContent)
	...
	Set fFile = FS.OpenTextFile(strFileName,ForAppending,AppendIfExists)
